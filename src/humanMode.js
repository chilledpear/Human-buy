...
async function humanMode(ca, minDelayMs, maxDelayMs, sellPct) {
...
async function delay(minDelay, maxDelay) {
    // Ensure we're using the correct bounds for random delay
    const actualMinDelay = Math.min(minDelay, maxDelay);
    const actualMaxDelay = Math.max(minDelay, maxDelay);
    
    // Calculate random delay within the specified range
    const delayTime = Math.floor(Math.random() * (actualMaxDelay - actualMinDelay + 1)) + actualMinDelay;
