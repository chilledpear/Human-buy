...
async function multipleSell(tokenCA, rl) {
    ...
    while (true) {
        ...
        const parsedIndices = indexList.map(num => parseInt(num, 10)).filter(num => !isNaN(num) && num >= 1 && num <= wallets.length);
    ...
    // Prompt user for minimum and maximum delay between transactions (in milliseconds)
    const minDelayInput = await promptUser(chalk.yellow('Enter minimum delay between transactions (ms): '));
    const maxDelayInput = await promptUser(chalk.yellow('Enter maximum delay between transactions (ms): '));
  ...
  while (isPaused) {
    ...
    await new Promise(resolve => setTimeout(resolve, 500));
        ...
        try {
            ...
            // Check SOL balance to ensure wallet can pay transaction fees
        ...
        // Wait for a random delay before processing the next wallet (if any)
